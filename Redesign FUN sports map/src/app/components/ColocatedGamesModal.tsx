...
import { X, MapPin, ChevronRight, Share2 } from "lucide-react";
...
type ColocatedGamesModalProps = {
  ...
  /** Host-only: set status=live. */
  onStartGame?: (game: GameRow) => Promise<void> | void;
...
export function ColocatedGamesModal({
  ...
  // "Active" is "hasn't finished", not a status allowlist — the old check listed
  // open/full/live and so quietly disagreed with the rest of the app about games whose
  // window had simply run out. `isGameEnded` is the shared answer.
  const activeCount = (list: GameRow[]) => list.filter((g) => !isGameEnded(g, now)).length;
